...
// Cache con chiave game+userId per evitare contaminazione cross-game
...
export default function BinderDetail() {
  ...
  const [ownedCardIds, setOwnedCardIds] = useState<Set<string>>(() => {
    if (!game || !user) return new Set();
    return _ownedCache.get(ownedCacheKey(game, user.id)) ?? new Set();
  });

  const loadOwned = useCallback(async () => {
    if (!game || !user) return;
    const key = ownedCacheKey(game, user.id);
    if (_ownedCache.has(key) && _ownedCache.get(key)!.size > 0) {
      setOwnedCardIds(_ownedCache.get(key)!);
      return;
    }
    ...
    if (error) return;
    const ids = new Set(
      (data ?? []).map((r: any) => r.card_id).filter(Boolean) as string[],
    );
    _ownedCache.set(key, ids);
    setOwnedCardIds(ids);
  }, [game, user?.id]);
  ...
  const load = useCallback(async () => {
    ...
    // Filtra per user_id per sicurezza
  ...
  return (
    <div>
      ...
      {/* Skeleton durante il caricamento degli slot */}
      {loading ? (
      ...
      {/* Dialog conferma rimozione pagina — sostituisce il confirm() nativo */}
      <Dialog open={confirmRemovePage} onOpenChange={setConfirmRemovePage}>
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-muted-foreground">
            Tutte le carte su questa pagina verranno eliminate dal binder. L'operazione non è reversibile.
          </p>
