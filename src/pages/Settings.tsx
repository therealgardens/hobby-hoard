    ...
    return (
      <div className="mt-4 rounded-md border bg-muted/30 p-3 space-y-2 text-sm">
        <div className="flex items-center gap-2 font-medium">
          {job.status === "running" && (
            <>
              <RefreshCw className="h-4 w-4 animate-spin text-primary" /> Sync in progress…
            </>
          )}
          {job.status === "succeeded" && (
            <>
              <CheckCircle2 className="h-4 w-4 text-green-600" /> Last sync succeeded
            </>
          )}
          {job.status === "failed" && (
            <>
              <AlertCircle className="h-4 w-4 text-destructive" /> Last sync failed
            </>
          )}
        ...
        {isAdmin && (
          <Card className="p-6">
            ...
            <p className="text-muted-foreground text-sm mb-4">
              The full card catalog is refreshed automatically every day at 3:00 UTC. Manual syncs run in the background
              and usually finish within 1–3 minutes.
            </p>
